...
class LRPModel(nn.Module):
    ...
    def forward_per_layer(self, x: torch.Tensor, topk=-1):
        activations = list()
        ...
        if topk != -1:
            ...
            #print("top_k_indices (first 3):", top_k_indices[:3])
            for index in top_k_indices:
                relevance_zero[..., index] = relevance[..., index]
            relevance = relevance_zero
            
            #print("nonzero per sample:", (relevance != 0).sum(dim=-1)[:10])

        relevances = []
        names = []
        
        block_names = []
        # OneWayResNet: layer1..4 liegen in features[4..7]
        features = self.model.features
        
        for L in [4,3,2,1]:
            layer_seq = features[L + 3]   # 1->4, 2->5, 3->6, 4->7
            for B in reversed(range(len(layer_seq))):
                block_names.append(f"layer{L}.{B}")
        ...
        for i, layer in enumerate(self.lrp_layers):
            ...
            # ---------- BASIC BLOCK ----------
            if "RelevancePropagationBasicBlockFlowsPureSkip" in layer.__class__.__name__:
                # ✅ Namen setzen, damit inner-Keys korrekt prefix't werden
                layer.block_name = block_names[bb_idx]
                bb_idx += 1

                relevance, inner = layer.forward(a, relevance, return_inner=True)

                for name, R in inner:
                    relevances.append(R.detach())
                    names.append(name)

            # ---------- NORMAL LAYER ----------
            else:
                # 1) R_out speichern (vor dem Schritt!)
                relevances.append(relevance.detach())
                names.append(f"{layer.__class__.__name__}.out.{i}")   

                # 2) Schritt: ergibt R_in
                relevance = layer.forward(a, relevance)

                # 3) R_in speichern
                relevances.append(relevance.detach())
                names.append(f"{layer.__class__.__name__}.in.{i}")

        return relevances, names
...
def basic_lrp_per_layer(
    model, image, rel_pass_ratio=1.0, topk=1, skip_connection_prop: SkipConnectionPropType = "latest"
):
    lrp_model = LRPModel(model, rel_pass_ratio=rel_pass_ratio, skip_connection_prop=skip_connection_prop)
    R = lrp_model.forward_per_layer(image, topk)
    return R
