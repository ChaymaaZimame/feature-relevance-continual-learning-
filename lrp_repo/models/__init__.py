...
class OneWayResNet(nn.Module):
    # init bekommt ein torchvision ResNet Model und baut daraus die Struktur, die LRPModel erwartet
    # indem in self.features die Convs, BNs, Relus, Maxpools und Layer1-4 in einer Sequential gespeichert werden
    # in self.avgpool wird die avgpool Schicht gespeichert
    # in self.classifier wird die finale Fully Connected Schicht in einer Sequential gespeichert
    ...
    # dann wird die forward Funktion definiert, die den Input durch features, avgpool und classifier leitet
