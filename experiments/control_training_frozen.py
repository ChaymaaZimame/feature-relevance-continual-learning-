...
if __name__ == "__main__":
    ...
    cfg.merge_from_file("configs_experiments/config_baseline_control_frozen.yaml")
    cfg.freeze()
    print(cfg)
    ...
    ##### 5.1 Save model after expand before training
    ...
    ####### 5.2 Freeze backbone and old heads, only train new heads
    old_num_classes = len(cfg.DATA.CLASS_INDICES_OLD)
    ...
    # save old fc weights and biases for copying them after every optimization step
    # so that the old heads remain unchanged during training
    old_fc_weight = model.model.fc.weight[:old_num_classes].detach().clone()
    
    if model.model.fc.bias is not None:
        old_fc_bias = model.model.fc.bias[:old_num_classes].detach().clone()
    else:
        old_fc_bias = None
