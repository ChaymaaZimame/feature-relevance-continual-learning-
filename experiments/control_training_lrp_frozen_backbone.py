...
from methods.lrp_scaler_only_fc_layer import LRPScaler
...
if __name__ == "__main__":
    ...
    cfg.merge_from_file("configs_experiments/config_baseline_control_lrp.yaml")
    cfg.freeze()
    print(cfg)
    ...
    lrp_runner = LRPRunner(model=model, device=device)
    # param grads scaling using relevance
    lrp_scaler = LRPScaler(beta=cfg.LRP.BETA, eps=1e-6, writer = writer)
    ...
    trainer = Trainer(
        ...
        lrp_runner=lrp_runner,
        lrp_scaler=lrp_scaler,
    )
    
    # prepare relevance computation (register hooks)
    trainer.prepare_lrp()
    ...
    train_losses, val_losses_new = trainer.update_model(
        ...
        scheduler=scheduler,
        log_every=10,
        lrp_mode=cfg.LRP.MODE,  # "once", "epoch" or "batch"
        lrp_topk=1,
        normalize=cfg.LRP.NORMALIZE,
