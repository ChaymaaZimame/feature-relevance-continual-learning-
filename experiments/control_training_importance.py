...
from methods.importance_scaler import ImportanceScaler
...
if __name__ == "__main__":
    ...
    cfg.merge_from_file("configs_experiments/config_baseline_control_importance.yaml")
    cfg.freeze()
    print(cfg)
    ...
    neuron_imp_calc = NeuronImportanceCalculator(delta=cfg.IMPORTANCE.DELTA) # i still initialize it with delta, but it is not used in the current implementation of update_importance (NO EMA)
    # param grads scaling using importance
    importance_scaler = ImportanceScaler(beta=cfg.IMPORTANCE.BETA, eps=1e-6, writer=writer)
    ...
    trainer = Trainer(
        ...
        neuron_imp_calc=neuron_imp_calc,
        importance_scaler=importance_scaler,
    )
    
    # prepare importance computation (register hooks)
    trainer.prepare_importance()
    ...
    train_losses, val_losses_new = trainer.update_model(
        ...
        scheduler=scheduler,
        log_every=10,
        importance_mode=cfg.IMPORTANCE.MODE,  # "once", "epoch" or "batch"
